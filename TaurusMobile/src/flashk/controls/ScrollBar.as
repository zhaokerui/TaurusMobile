package flashk.controls
{
	...
	import flashk.display.IScrollContent;
	import flashk.events.TickEvent;
	import flashk.events.TweenEvent;
	import flashk.utils.CircUtil;
	import flashk.utils.FilterProxy;
	...
	public class ScrollBar extends Silder
	{
		 ...
		 * 滚动缓动效果
		 */
		public var easing:Function = CircUtil.easeOut;
		 ...
		 * 滑动块自适应
		 */
		public var autoThumbSize:Boolean = false;
		
		
		public function ScrollBar(skin:*=null, replace:Boolean=true,fields:Object=null)
		{
		...
		public function set target(v:*):void
		{
			...
			else if (_target is Text)
				_scrollContent = new ScrollTextContent((_target as Text).textField);
		...
		protected function setThumbSize():void
		{
			if (_scrollContent)
			{
				if (direction == UIConst.HORIZONTAL)
				{
					thumbAreaLength = (downArrow ? downArrow.x : this.width) - thumbAreaStart;
					thumb.width = thumbAreaLength/maxPage;
					thumbAreaLength -= thumb.width;
				}else{					
					thumbAreaLength = (downArrow ? downArrow.y : this.height) - thumbAreaStart;
					thumb.height = thumbAreaLength/maxPage;
					thumbAreaLength -= thumb.height;
		...
		protected function thumbMouseWheelHandler(evt:MouseEvent):void
		{
			if (direction == UIConst.HORIZONTAL)
			{
				if(evt.delta>0)
				{
					...
					upArrowClickHandler(null);
				} else {
					thumb.x += thumb.mouseX;
					downArrowClickHandler(null);
				}
