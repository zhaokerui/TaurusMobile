package flashk.display
{
	...
	import flashk.events.MovieEvent;
	...
	[Event(name="movie_start",type="flashk.events.MovieEvent")]
	
	[Event(name="movie_end",type="flashk.events.MovieEvent")]
	
	[Event(name="movie_empty",type="flashk.events.MovieEvent")]
	
	
	/**
	 * 作为MovieClip和BitmapMovieClip共同的基类。此类不允许实例化。
	 * 
	 * @author flashyiyi
	 * 
	 */
	public class KMovieClip extends UIBase
	{
		...
		public function KMovieClip(skin:*=null, replace:Boolean=true, paused:Boolean=false)
		{
			AbstractUtil.preventConstructor(this,KMovieClip);
			super(skin, replace);
		...
		public function setLabel(labelName:String, repeat:int=-1, clearQueue:Boolean = true):void
		{
			...
			if (index != -1)
			{
				...
				if (KMovieClip.labelHandlers[labelName])
					(KMovieClip.labelHandlers[labelName] as Handler).call();
			}
			else
			{
		...
		public function set currentFrame(frame:int):void
		{
			...
			if (linkMovieClips)
			{
				for each (var mc:KMovieClip in linkMovieClips)
		 ...
		 * 
		 */
		public function linkTo(target:KMovieClip):void
		{
			this.paused = true;
		 ...
		 * 
		 */
		public function removeLinkFrom(target:KMovieClip):void
		{
			this.paused = false;
			DataUtil.remove(target.linkMovieClips,this);
