	...
	public final class LayoutUtil
	{
		...
		public static function metrics(obj:*,container:*,left:Number=NaN,top:Number=NaN,right:Number=NaN,bottom:Number=NaN):void
		{
			...
			if (!isNaN(right))
			{
				if (!isNaN(left))
				{
					if (obj is UIBase)//使用setSize方法避免重复触发Resize事件
						(obj as UIBase).setSize(cRect.right - (obj.x - offest.x) - right,obj.height);
					else
			...
			if (!isNaN(bottom))
			{
				...
				{
					if (obj is UIBase)
						(obj as UIBase).setSize(obj.width,cRect.bottom - (obj.y - offest.y) - bottom);
					else
