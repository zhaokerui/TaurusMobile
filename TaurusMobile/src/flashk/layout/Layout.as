package flashk.layout
{
	...
	public class Layout
	{
		...
		public function setTarget(value:DisplayObjectContainer,isRoot:Boolean = false):void
		{
			if (_target)
			{
				_target.removeEventListener(KEvent.CHILD_RESIZE,childResizeHandler);
				if (_target is UIBase && (_target as UIBase).content)
					(_target as UIBase).content.removeEventListener(KEvent.CHILD_RESIZE,childResizeHandler);
				
				_target.removeEventListener(KEvent.RESIZE,resizeHandler);
				if (_target.stage)
			...
			if (_target)
			{
				_target.addEventListener(KEvent.CHILD_RESIZE,childResizeHandler);
				if (_target is UIBase && (_target as UIBase).content)
					(_target as UIBase).content.addEventListener(KEvent.CHILD_RESIZE,childResizeHandler);
				
				if (isRoot)
				{
					if (_target.stage)
						_target.stage.addEventListener(KEvent.RESIZE,resizeHandler);
				}
				else
					_target.addEventListener(KEvent.RESIZE,resizeHandler);
			}
		...
		protected function measureChildren(width:Number = NaN,height:Number = NaN):void
		{
			...
			if (target is UIBase)
			{
				(target as UIBase).setSize(width,height,true);//避免重复触发
			}
			else
			{
				target.width = width;
